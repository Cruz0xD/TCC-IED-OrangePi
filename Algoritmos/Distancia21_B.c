...
// Convencional: gcc -Wall -Iincludes_libiec61850 -Llibiec61850-1.5.1/build Algoritmos/Distancia21_B.c Algoritmos/goose_sub.c Algoritmos/fft.c Algoritmos/phasor.c Algoritmos/mergingUnit.c -liec61850 -o IED_B -lm -lpthread
// Orange Pi: gcc -Wall -Iincludes_libiec61850 -Llibiec61850-1.5.1/build Algoritmos/Distancia21_B.c Algoritmos/goose_sub.c Algoritmos/fft.c Algoritmos/phasor.c Algoritmos/mergingUnit.c -liec61850 -o IED_B -lm -lpthread -lwiringPi
// Para executar: sudo chrt -f 80 taskset -c 2 ./IED_B

#include "mergingUnit.h"
#include "phasor.h"
#include "goose_sub.h"  // O IED B apenas escuta as mensagens GOOSE
...
volatile int trip = 0; // Sinal global de TRIP do relé local
volatile int* trip_ptr = &trip;

volatile int tripA = 0; // Sinal global de TRIP vindo do relé no barramento A
volatile int* tripA_ptr = &tripA;
...
// Thread que executa a escuta GOOSE contínua
void* goose_thread_listener(void* arg) {
    goose_listen_loop("end0");  // Mude a interface Ethernet conforme necessário
    return NULL;
}
...
int main() {
    ...
    // Defina o horário de início desejado (24h format), deve ser o mesmo horário definido no IED_A
    int start_hour = 20;
    int start_min = 10;
    int start_sec = 0;
    ...
    */
    ...
    pthread_t goose_thread;
    // Configura o ponteiro global usado pelo listener
    set_trip_ptr(&tripA);
    // Cria a thread para rodar o listener GOOSE contínuo
    if (pthread_create(&goose_thread, NULL, goose_thread_listener, NULL) != 0) {
        perror("Erro ao criar thread GOOSE listener");
        return 1;
    }
    ...
    while (1) {
        ...
        if(*tripA_ptr == 1){
            trip_("Falta detectada pelo relé no barramento A - Zona 1!");
        }
    ...
    // pthread_join(goose_thread, NULL);
    return 0;
}
